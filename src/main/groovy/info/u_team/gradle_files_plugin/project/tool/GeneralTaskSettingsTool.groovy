		...
		if(GradleFilesUtil.isMainProject(project)) {
			project.gradle.taskGraph.whenReady { TaskExecutionGraph graph ->
				final def hasPublishTask = graph.allTasks.any { task ->
					task.name.startsWith(Constants.PUBLISH_EXTERNAL_TASK) || task.name.startsWith(Constants.CURSEFORGE_EXTERNAL_TASK) || task.name.startsWith(Constants.PUBLISH_MODS_EXTERNAL_TASK)
				}
